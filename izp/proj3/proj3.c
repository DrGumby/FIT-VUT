...
float cluster_distance(struct cluster_t *c1, struct cluster_t *c2)
{
    ...
    else if(premium_case == 2)
    {
        ...
        for (int i = 0; i < c1->size; ++i) {
            for (int j = 0; j < c2->size; ++j) {
                tempMin = obj_distance(&c1->obj[i], &c2->obj[j]);
                if(tempMin < min)
                {
                    min = tempMin;
                }
    ...
    else
    {
        ...
        for (int i = 0; i < c1->size; ++i) {
            for (int j = 0; j < c2->size; ++j) {
                tempMax = obj_distance(&c1->obj[i], &c2->obj[j]);
                if(tempMax > max)
                {
                    max = tempMax;
                }
...
int load_clusters(char *filename, struct cluster_t **arr) {
    ...
    int y = 0;
    int i = 0;
    if (fscanf(objectList, "count=%d", &count) != 1 || count == 0)
    {
        fclose(objectList);
...
int main(int argc, char *argv[])
{
    ...
    if(argc > 1)
    {
        ...
        if(argc == 4)
        {
            ...
            {
                fprintf(stderr, "Unsupported method. Exiting...\n");
                for (int i = 0; i < clusterCountTotal; ++i) {
                    remove_cluster(&clusters[i], 1, 0);
                }
                free(clusters);
